 ...
 * This class was originally implemented for Euler Problem #13
 * The primary functions include addition, multiplication, and exponentiation.
 * 
 * TODO: subtraction, division, and negative numbers
 * 
 * @author Trevor Tsai
 * @version 2/20/20
 */
...
public class BigNum implements Comparable<BigNum>{
	...
	public BigNum(String in){
		...
		for(int i = 0; i < input.length(); i++){
			if(!Character.isDigit(input.charAt(i))){
				throw new IllegalArgumentException("The String should represent" + " a positive number");
			}
		}
	...
	public BigNum add(long input){
		if(input < 0){
			throw new IllegalArgumentException("Must be a natural number " + "(greater than 0)");
		}
	...
	private String parseTrailingZeroes(String input){
		...
		String output = input;
		if(zeroCounter > 0){
			output = output.substring(0, output.indexOf("E") - zeroCounter) + output.substring(output.indexOf("E"));
		}
		if(output.contains(".E")){
			output = output.substring(0, output.indexOf(".E")) + output.substring(output.indexOf("E"));
		}
		return output;
	}
	...
	public String toScientific(int num, boolean trailingZeroes){
		if(num <= 0){
			throw new IllegalArgumentException("Need 1 or more digits of " + "precision");
		}
