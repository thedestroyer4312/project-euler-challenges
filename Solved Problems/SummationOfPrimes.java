...
public class SummationOfPrimes{
  ...
  *This only works for n <= INTEGER.MAX_VALUE because the maximum length of an array is that value
  *To go above this value, another implementation would be required to exceed the integer limit
 ...
 /**
  *Finds all prime numbers under n using the Sieve of Eratosthenes
  *The Sieve of Eratosthenes is where once a prime number is found, all multiples of that number are then eliminated as primes
  *@param n The upper limit for primes
  ...
  */
  public static int[] findPrimesUnder(long n){
    ...
    for(int i = 0; i < list.length; i++){//Setting all values of list to true initially (prime until proven otherwise)
      list[i] = true;
    }
 ...
 /**
  *Finds whether a number is prime using the fact that all primes can be written as 6k +- 1
